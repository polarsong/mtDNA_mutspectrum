...
df_codons_trophl = df_sgc %>% select(species_name, gene_name, trophic_level,all_of(vec_all))
...
for (org in 1:nrow(df_try))
{
  ...
  sp_out = data.frame(sp_r$species_name, med_c, med_a, sp_r$trophic_level) 
  final = rbind(final,sp_out)
}
names(final) = c('species_name', 'med_c', 'med_a', 'trophic_level')
...
violin_c = ggplot(final, aes(x = trophic_level, y = med_c)) +
  geom_violin()
violin_a = ggplot(final, aes(x = trophic_level, y = med_a)) +
  geom_violin()
box_c = ggplot(final, aes(x = trophic_level, y = med_c)) +
  geom_boxplot()
box_a = ggplot(final, aes(x = trophic_level, y = med_a)) +
  geom_boxplot()
