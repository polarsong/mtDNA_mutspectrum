...
SynNuc = SynNuc[SynNuc$TAXON == 'Amphibia',]

############ merge with ecology
SynNucAA = merge(AA,SynNuc)
length(unique(SynNucAA$Species))  # 33 species
...
cor.test(log2(AGG$MLS),AGG$FrT) # nothing
cor.test(log2(AGG$MLS),AGG$FrG) # nothing
cor.test(log2(AGG$MLS),AGG$FrC) # nothing

# plot it:

pdf("./Body/4Figures/APMutSpectrum.SynSites.MutagensWithinAmphibia.R.01.pdf", width = 50, height = 30)
