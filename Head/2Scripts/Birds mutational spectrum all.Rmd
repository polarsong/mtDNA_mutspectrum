...
#no mistake data_frame
df_mistake = df_finale
df_mistake$omnivoretn1 = NA
df_mistake$insessorial1 = NA
df_mistake = df_mistake[, colSums(is.na(df_mistake)) < nrow(df_mistake)]
tree_all = treedata(phy, df_mistake, sort = T, warnings = T)$phy
all_pgls = as.data.frame(treedata(tree_all, df_mistake, sort = T, warnings = T)$data)
MutComp_all = comparative.data(tree_all, all_pgls, species_name, vcv = TRUE)
...
for (i in vec1)
{
  for (j in vec2) 
  {
    if (i != j)
    {
      model_all = pgls(as.numeric(as.character(MutComp_all$data[,i])) ~ as.numeric(as.character(MutComp_all$data[,j])),  MutComp_all, lambda = 'ML')
      p = summary(model_all)$coefficients[,4][2]
      r = summary(model_all)$r.squared
      ef = summary(model_all)$coefficients[,1][2]
