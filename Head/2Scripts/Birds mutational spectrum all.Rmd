...
phy=multi2di(tree)
df_pgls = df_mtdna[c(1,2,5,96,97,98,99,100,103,104,105,106,107,108,109,110,111,112,113,114,115,120:158)]
#solve problems with multiple genes
row.names(df_pgls) = df_pgls$species_name
df_pgls$gene_name = NA
df_pgls = df_pgls[, colSums(is.na(df_pgls)) < nrow(df_pgls)]
tree_all = treedata(phy, df_pgls, sort = T, warnings = T)$phy
all_pgls = as.data.frame(treedata(tree_all, df_pgls, sort = T, warnings = T)$data)
...
for (i in vec1)
{
  for (j in vec2) 
  {
    ...
    p = model_all$coefficients[,4][2]
    r = model_all$r.squared
    ef = model_all$coefficients[,1][2]
